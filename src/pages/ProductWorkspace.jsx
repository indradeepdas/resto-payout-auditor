...
const recoverySummaryCards = [
  { key: 'totalDiscrepancyAmount', label: 'Total discrepancy amount', format: formatCurrency },
  { key: 'estimatedRecoverableAmount', label: 'Estimated recoverable', format: formatCurrency },
  { key: 'highConfidenceClaimCount', label: 'High-confidence claim lines', format: (value) => value },
  { key: 'topRecoverabilityIssueType', label: 'Top recoverability issue type', format: (value) => value },
];

const emptyStateMessage =
  'Upload both files or use demo data to generate an audit. All processing still stays in the browser.';
const getFileType = (fileName = '') => fileName.split('.').pop()?.toLowerCase() || 'unknown';
const auditTrackingState = {
  started: new Set(),
  succeeded: new Set(),
  failed: new Set(),
  recoveryGenerated: new Set(),
};
...
const resetAuditTracking = () => {
  ...
  auditTrackingState.recoveryGenerated.clear();
};
...
export default function ProductWorkspace() {
  ...
  const recoveryRows = useMemo(() => scoreRecoveryIssues(issueRows), [issueRows]);
  const recoverySummary = useMemo(() => buildRecoverySummary(recoveryRows), [recoveryRows]);
  const highConfidenceClaims = useMemo(() => getHighConfidenceClaims(recoveryRows), [recoveryRows]);
  ...
  const baseRecoveryProperties = useMemo(
    () => ({
      discrepancy_amount_eur: recoverySummary.totalDiscrepancyAmount,
      estimated_recoverable_amount_eur: recoverySummary.estimatedRecoverableAmount,
      high_confidence_claim_count: recoverySummary.highConfidenceClaimCount,
      issue_count: recoverySummary.issueCount,
      top_issue_type: recoverySummary.topRecoverabilityIssueType,
    }),
    [recoverySummary],
  );
  ...
  const recoverySignature = useMemo(
    () =>
      summary
        ? JSON.stringify({
            auditSignature,
            discrepancy: recoverySummary.totalDiscrepancyAmount,
            recoverable: recoverySummary.estimatedRecoverableAmount,
            highConfidence: recoverySummary.highConfidenceClaimCount,
            issueCount: recoverySummary.issueCount,
          })
        : '',
    [auditSignature, recoverySummary, summary],
  );
  ...
  useEffect(() => {
    if (!summary || !recoverySignature || auditTrackingState.recoveryGenerated.has(recoverySignature)) {
      return;
    }

    auditTrackingState.recoveryGenerated.add(recoverySignature);
    trackEvent('recovery_summary_generated', {
      ...baseAuditProperties,
      ...baseRecoveryProperties,
      orders_analyzed: summary.ordersAnalyzed,
      issues_found: summary.issuesFound,
    });
  }, [baseAuditProperties, baseRecoveryProperties, recoverySignature, summary]);
  ...
  const handleGenerateClaimPack = () => {
    if (!summary) {
      return;
    }

    const nextClaimPack = generateClaimPack({
      recoveryRows,
      auditSummary: summary,
    });

    setClaimPack(nextClaimPack);
    trackEvent('claim_pack_generated', {
      ...baseAuditProperties,
      ...baseRecoveryProperties,
      orders_analyzed: summary.ordersAnalyzed,
      issues_found: summary.issuesFound,
  ...
  const downloadClaimPackJson = () => {
    if (!claimPack) {
      return;
    }

    trackEvent('claim_pack_json_downloaded', {
      ...baseAuditProperties,
      ...baseRecoveryProperties,
      orders_analyzed: summary?.ordersAnalyzed,
      issues_found: summary?.issuesFound,
    ...
    const blob = new Blob([JSON.stringify(claimPack, null, 2)], {
      type: 'application/json;charset=utf-8;',
    });

    triggerDownload('uber-eats-claim-pack.json', blob);
  };

  const downloadClaimPackCsv = () => {
    if (!claimPack) {
      return;
    }

    trackEvent('claim_pack_csv_downloaded', {
      ...baseAuditProperties,
      ...baseRecoveryProperties,
      orders_analyzed: summary?.ordersAnalyzed,
      issues_found: summary?.issuesFound,
    ...
    exportToCsv('uber-eats-claim-pack.csv', claimPackToCsvRows(claimPack));
  };
  ...
  const claimPackStatus = claimPack
    ? `${claimPack.claim_lines.length} claim lines packaged`
    : 'Claim pack not generated';
      ...
      <main className="page">
        <header className="workspace-topbar card">
          <div>
            ...
            <h1>Payout Recovery Workspace</h1>
            <p>Signed in as {user?.email}. Audit payouts, estimate recovery value, and package dispute evidence.</p>
          </div>
          <div className="workspace-actions">
            <Link className="secondary-button auth-link-button" to="/account/privacy">
              Privacy center
            </Link>
        ...
        <section className="hero card hero-card">
          <div className="hero-copy">
            <span className="eyebrow">Client-side payout recovery</span>
            <h2>Uber Eats Payout Recovery Engine</h2>
            <p>
              Upload your Uber payout export and POS order file to detect discrepancies,
              estimate recoverable revenue, and generate a dispute-ready claim pack.
            </p>
          </div>
        ...
        <section className="controls-grid">
          ...
          <div className="card export-card">
            <h2>Export audit</h2>
            <p>Keep the raw reconciliation output alongside the new recovery claim pack.</p>
        ...
        <section className="summary-grid recovery-summary-grid">
          {recoverySummaryCards.map((card) => (
            <div className="card summary-card recovery-card" key={card.key}>
              <span>{card.label}</span>
              <strong>{summary ? card.format(recoverySummary[card.key]) : '--'}</strong>
            </div>
          ))}
        </section>
        ...
        <section className="card recovery-panel">
          <div className="section-header">
            <div>
              ...
              <p>Translate detected payout issues into recoverable claim lines and dispute actions.</p>
            </div>
            <span className="chip chip-accent">{claimPackStatus}</span>
          </div>

          <div className="recovery-layout">
            <div className="recovery-callout">
              <span className="eyebrow recovery-eyebrow">Recovery opportunity</span>
              <strong>{summary ? formatCurrency(recoverySummary.totalDiscrepancyAmount) : '--'}</strong>
              <p>
                {summary
                  ? `${formatCurrency(recoverySummary.estimatedRecoverableAmount)} is likely recoverable based on deterministic dispute rules.`
                  : 'Run the audit to estimate how much payout discrepancy is commercially worth pursuing.'}
              </p>
              ...
              <div className="recovery-actions">
                <button
                  className="primary-button"
                  type="button"
                  onClick={handleGenerateClaimPack}
                  disabled={!summary}
                >
                  Generate Claim Pack
                </button>
                <button
                  className="secondary-button"
                  type="button"
                  onClick={downloadClaimPackJson}
                  disabled={!claimPack}
                >
                  Download Claim Pack JSON
                </button>
                <button
                  className="secondary-button"
                  type="button"
                  onClick={downloadClaimPackCsv}
                  disabled={!claimPack}
                >
                  Download Claim Pack CSV
                </button>
              </div>
            </div>
            ...
            <div className="recovery-side">
              <div className="recovery-note">
                <span className="recovery-note-label">Recommended next step</span>
                <strong>
                  {summary ? recoverySummary.recommendedNextStep : 'Audit data is required first.'}
                </strong>
              </div>

              <div className="recovery-list">
                <div className="recovery-list-header">
                  <strong>High-confidence claim lines</strong>
                  <span>{summary ? highConfidenceClaims.length : 0}</span>
                </div>

                {summary ? (
                  highConfidenceClaims.length ? (
                    highConfidenceClaims.slice(0, 5).map((row) => (
                      <div className="claim-line-item" key={`${row.orderId}-${row.issue_type}`}>
                        <div>
                          <strong>{row.orderId}</strong>
                          <span>{row.issue_type}</span>
                        </div>
                        <span>{formatCurrency(row.estimated_recoverable_amount_eur)}</span>
                      </div>
                    ))
                  ) : (
                    <p className="recovery-empty">No high-confidence claim lines were identified for this audit.</p>
                  )
                ) : (
                  <p className="recovery-empty">Load both files or use demo data to preview the claim-line prioritization.</p>
                )}
              </div>
            </div>
          </div>
